...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/attendance/pending-regularizations", requireAuth, async (req, res) => {
    try {
      const userId = req.session.userId!;
      
      // Get all employees who report to this user (manager)
      const directReports = await storage.getAllUserProfiles();
      const reportIds = directReports
        .filter(emp => emp.managerId === userId)
        .map(emp => emp.id);
      ...
      const pendingRegularizations = allAttendance.filter(att => 
        reportIds.includes(att.userId) && 
        att.regularizationRequested && 
        att.regularizationStatus === 'Pending'
      );

      // Enrich with employee details
      const enriched = await Promise.all(
        pendingRegularizations.map(async (att) => {
          const employee = await storage.getUserProfile(att.userId);
          return {
            ...att,
      ...
      res.status(500).json({ message: "Failed to fetch pending regularizations" });
    }
  });

  app.patch("/api/attendance/:id/approve-regularization", requireAuth, async (req, res) => {
    try {
      ...
      const attendance = await storage.updateAttendance(id, {
        ...
        regularizationApprovedBy: managerId,
      ...
      res.status(500).json({ message: error.message || "Failed to approve regularization" });
    }
  });

  app.patch("/api/attendance/:id/reject-regularization", requireAuth, async (req, res) => {
    try {
      const managerId = req.session.userId!;
      const { id } = req.params;
      ...
      res.status(500).json({ message: error.message || "Failed to reject regularization" });
    }
  });
